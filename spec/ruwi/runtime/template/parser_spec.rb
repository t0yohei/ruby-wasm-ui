...
RSpec.describe Ruwi::Template::Parser do
  ...
  describe '.parse_and_eval' do
    ...
    context 'when evaluating a template with variables' do
      it 'returns a VDOM object with evaluated variables' do
        ...
        # Mock Ruwi::Vdom
        stub_const('Ruwi::Vdom', mock_vdom)
    ...
    context 'when evaluating multiple top-level expressions' do
      it 'wraps multiple expressions in a fragment' do
        ...
        allow(described_class).to receive(:eval).with('Ruwi::Vdom.h_fragment([expr1,expr2])', anything).and_return(mock_fragment)
      ...
      it 'wraps expressions containing end, in a fragment' do
        ...
        # Mock eval to return the fragment
        mock_fragment = double('fragment')
        allow(described_class).to receive(:eval).with('Ruwi::Vdom.h_fragment([if condition then expr1 end,expr2])', anything).and_return(mock_fragment)
  ...
  describe 'integration tests for preprocessing chain' do
    ...
    context 'when processing r-for attributes in templates' do
      ...
      before do
        ...
        allow(Ruwi::Template::BuildConditionalGroup).to receive(:has_conditional_attribute?).with(mock_element).and_return(false)
        allow(Ruwi::Template::BuildForGroup).to receive(:has_for_attribute?).with(mock_element).and_return(true)
        allow(Ruwi::Template::BuildForGroup).to receive(:build_for_loop).with(mock_element).and_return("todos.map do |todo|\n  Ruwi::Vdom.h(TodoItemComponent, {:key => todo[:text]}, [])\nend")
      end
      ...
      it 'evaluates r-for templates with variables' do
        ...
        expect(result).to include('Ruwi::Vdom.h(TodoItemComponent')
