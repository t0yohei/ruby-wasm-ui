...
RSpec.describe Ruwi::Template::BuildForGroup do
  ...
  describe '.build_for_loop' do
    ...
    context 'when element has valid r-for expression' do
      ...
      before do
        ...
        allow(Ruwi::Template::BuildVdom).to receive(:embed_script?).with('{todo in todos}').and_return(true)
        allow(Ruwi::Template::BuildVdom).to receive(:get_embed_script).with('{todo in todos}').and_return('todo in todos')
        allow(Ruwi::Template::BuildVdom).to receive(:is_component?).with('todo-item-component').and_return(true)
        allow(Ruwi::Template::BuildVdom).to receive(:parse_attributes).and_return(':key => todo[:text], :todo => todo[:text], :id => todo[:id]')
        allow(Ruwi::Template::BuildVdom).to receive(:build).with(mock_child_nodes).and_return('')
      end

      it 'generates map code for component' do
        result = described_class.build_for_loop(mock_element)
        expected = "todos.map do |todo|\n  Ruwi::Vdom.h(TodoItemComponent, {:key => todo[:text], :todo => todo[:text], :id => todo[:id]}, [])\nend"
    ...
    context 'when element has r-for expression without curly braces' do
      ...
      before do
        ...
        allow(Ruwi::Template::BuildVdom).to receive(:embed_script?).with('item in items').and_return(false)
        allow(Ruwi::Template::BuildVdom).to receive(:is_component?).with('todo-item-component').and_return(true)
        allow(Ruwi::Template::BuildVdom).to receive(:parse_attributes).and_return('')
        allow(Ruwi::Template::BuildVdom).to receive(:build).with(mock_child_nodes).and_return('')
      end

      it 'generates map code without script extraction' do
        result = described_class.build_for_loop(mock_element)
        expected = "items.map do |item|\n  Ruwi::Vdom.h(TodoItemComponent, {}, [])\nend"
        expect(result).to eq(expected)
      end
    end
    ...
    context 'when element is a regular HTML element' do
      before do
        ...
        allow(Ruwi::Template::BuildVdom).to receive(:embed_script?).with('{item in items}').and_return(true)
        allow(Ruwi::Template::BuildVdom).to receive(:get_embed_script).with('{item in items}').and_return('item in items')
        allow(Ruwi::Template::BuildVdom).to receive(:is_component?).with('li').and_return(false)
        allow(Ruwi::Template::BuildVdom).to receive(:parse_attributes).and_return('')
        allow(Ruwi::Template::BuildVdom).to receive(:build).with(mock_child_nodes).and_return('')
      end
    ...
    context 'when r-for expression has invalid syntax' do
      ...
      before do
        ...
        allow(Ruwi::Template::BuildVdom).to receive(:embed_script?).with('invalid syntax').and_return(false)
      end
  ...
  describe '.build_component_for_item' do
    ...
    before do
      allow(mock_element).to receive(:[]).with(:childNodes).and_return(mock_child_nodes)
      allow(mock_child_nodes).to receive(:[]).with(:length).and_return(0)
      allow(Ruwi::Template::BuildVdom).to receive(:parse_attributes).with(filtered_attributes).and_return(':key => todo[:text], :todo => todo[:text], :id => todo[:id]')
      allow(Ruwi::Template::BuildVdom).to receive(:build).with(mock_child_nodes).and_return('')
    end
    ...
    it 'builds component with PascalCase name conversion' do
      ...
      expect(result).to eq("Ruwi::Vdom.h(TodoItemComponent, {:key => todo[:text], :todo => todo[:text], :id => todo[:id]}, [])")
    end
    ...
    it 'builds multi-word component with PascalCase name conversion' do
      ...
      expect(result).to eq("Ruwi::Vdom.h(MyCustomButton, {:key => todo[:text], :todo => todo[:text], :id => todo[:id]}, [])")
    end
    ...
    it 'builds single word component' do
      ...
      expect(result).to eq("Ruwi::Vdom.h(Mycomponent, {:key => todo[:text], :todo => todo[:text], :id => todo[:id]}, [])")
    end
  end
  ...
  describe '.build_element_for_item' do
    ...
    before do
      allow(mock_element).to receive(:[]).with(:childNodes).and_return(mock_child_nodes)
      allow(mock_child_nodes).to receive(:[]).with(:length).and_return(0)
      allow(Ruwi::Template::BuildVdom).to receive(:parse_attributes).with(filtered_attributes).and_return(':class => \'item-class\', :id => item.id')
      allow(Ruwi::Template::BuildVdom).to receive(:build).with(mock_child_nodes).and_return('')
    end

    it 'builds regular HTML element' do
      result = described_class.build_element_for_item(mock_element, 'li', filtered_attributes, 'item')
      expect(result).to eq("Ruwi::Vdom.h('li', {:class => 'item-class', :id => item.id}, [])")
    end
    ...
    it 'builds HTML element with lowercase tag name' do
      ...
      expect(result).to eq("Ruwi::Vdom.h('div', {:class => 'item-class', :id => item.id}, [])")
    end
  end
end
